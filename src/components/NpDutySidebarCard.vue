...
<template>
  <section class="mt-3 rounded-xl border border-accent/20 bg-accent/[0.05] p-2.5" aria-label="NP 值班">
    ...
    <!-- 這天完全沒有資料：多半是本月尚未匯入 -->
    <div v-else-if="!rows.length" class="space-y-1.5 text-xs">
      <p class="text-muted">尚無值班資料</p>
      ...
      <p v-else class="text-2xs text-muted leading-snug">可在「資料管理 → 值班 NP」設定班表下載網址</p>
    </div>

    <div v-else class="space-y-1.5">
      <div v-for="ward in WARDS" :key="ward" class="flex items-start gap-2 text-xs">
