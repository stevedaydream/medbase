...
onTableSynced("surgery", onSynced);
...
async function save() {
  ...
  await reload();
  selected.value = items.value.find((m) => m.id === prevId) ?? null;
  await touchTable("surgery");
}

async function deleteSelected() {
  if (!selected.value) return;
  const db = await getDb();
  await markDeletedById("surgery", selected.value.id);
  ...
  <div class="accent-teal flex gap-6 h-full text-fg select-none bg-sunken">
    ...
    <div class="flex flex-col w-80 shrink-0 bg-surface border border-hairline rounded-2xl p-4 shadow-xl overflow-hidden">
      ...
      <div class="flex items-center justify-between px-1.5 mb-3 shrink-0">
        ...
        <div class="flex gap-1">
          <CloudSyncButtons table="surgery" @synced="onSynced" @message="toast" />
        </div>
      </div>
      ...
      <div class="flex-1 overflow-y-auto space-y-1.5 pr-1 custom-scrollbar">
        ...
        <button v-for="m in filtered" :key="m.id"
          ...
          :class="selected?.id === m.id 
            ? 'bg-accent/20 border-accent/50 text-white shadow-[0_0_15px_rgba(249,115,22,0.08)]' 
          ...
          <div class="text-2xs font-mono mt-1 flex items-center gap-1.5 opacity-65">
            <span v-if="m.category" class="bg-sunken px-1.5 py-0.5 rounded text-accent font-semibold">{{ m.category }}</span>
          </div>
        </button>
      </div>
    </div>
    ...
    <div class="flex-1 rounded-2xl bg-surface border border-hairline p-6 overflow-y-auto flex flex-col shadow-xl">
      ...
      <div v-else class="space-y-6 flex-1 flex flex-col">
        ...
        <!-- 兩個 Tab -->
        <nav class="flex gap-1 p-1 bg-sunken rounded-xl border border-hairline shrink-0 shadow-inner max-w-sm">
          <button @click="activeTab = 'pre'"
            class="flex-1 py-2 rounded-lg text-xs font-black transition-all cursor-pointer"
            :class="activeTab === 'pre' ? 'bg-elevated text-accent shadow border border-hairline' : 'text-muted hover:text-fg-secondary'">
          ...
          <button @click="activeTab = 'post'"
            class="flex-1 py-2 rounded-lg text-xs font-black transition-all cursor-pointer"
            :class="activeTab === 'post' ? 'bg-elevated text-accent shadow border border-hairline' : 'text-muted hover:text-fg-secondary'">
