  ...
  <div class="accent-teal flex gap-6 h-full p-1 overflow-hidden">
    ...
    <div class="flex flex-col w-80 shrink-0 bg-sunken border border-hairline rounded-2xl p-4 shadow-xl overflow-hidden">
      ...
      <div class="flex gap-2 mb-3 shrink-0">
        <div class="relative flex-1">
          <span class="absolute left-3.5 top-2.5 text-muted text-xs">🔍</span>
          <input
            v-model="search"
            placeholder="搜尋名稱、分類…"
      ...
      <div class="flex-1 overflow-y-auto space-y-1.5 pr-1 custom-scrollbar">
        ...
        <button v-for="m in filtered" :key="m.id" @click="selected = m"
          ...
          <div class="flex items-center gap-1.5 mt-1.5">
            ...
            <span v-if="m.indication" class="text-2xs text-muted truncate flex-1 font-mono font-medium">{{ m.indication }}</span>
          </div>
        </button>
      </div>
    </div>
    ...
    <div class="flex-1 rounded-2xl bg-surface border border-hairline p-6 overflow-y-auto shadow-2xl flex flex-col min-w-0 custom-scrollbar">
      ...
      <div v-else class="space-y-6">
        <!-- Detail Header -->
        <div class="flex items-start justify-between pb-5 border-b border-hairline">
        ...
        <div class="bg-overlay/[0.02] border border-hairline rounded-xl p-5">
          ...
          <ol class="space-y-3.5">
            ...
            <li v-if="parseSteps(selected.orders).length === 0"
              class="text-muted text-xs text-center py-8 italic font-bold">
              尚無步驟，點「✏️ 編輯」新增
            </li>
          </ol>
        </div>
      ...
      <div v-if="showModal"
        ...
        <div class="w-full max-w-lg bg-surface border border-hairline shadow-2xl rounded-2xl flex flex-col max-h-[90vh] overflow-hidden text-fg">
          <div class="flex items-center justify-between px-5 py-4 border-b border-hairline bg-sunken shrink-0">
            ...
            <button @click="showModal = false" class="text-muted hover:text-fg text-xl leading-none transition-colors cursor-pointer">×</button>
          </div>
    ...
    <Teleport to="body">
      <div v-if="showDeleteConfirm"
        ...
        <div class="bg-surface border border-hairline rounded-2xl shadow-2xl p-6 w-full max-w-sm text-center text-fg">
          <div class="text-3xl mb-3">🗑️</div>
          <p class="text-sm font-bold mb-1">確認刪除配製參考？</p>
          <p class="text-xs text-muted mb-5 leading-normal">確定刪除「{{ selected?.name }}」？此操作將無法復原。</p>
