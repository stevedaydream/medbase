...
const BACKUP_GROUPS: BackupGroup[] = [
  ...
  { key: "scheduler",  label: "排班系統",   icon: "📅", tables: ["scheduler_users","sched_docs","app_settings"],desc: "排班使用者、排班 v3 文件與班表設定" },
...
const TABLE_LABELS: Record<string, string> = {
  ...
  physicians:"通訊錄",
  scheduler_users:"排班使用者", sched_docs:"排班 v3 文件", app_settings:"程式設定",
      ...
      <div v-if="activeTab === 'backup'" class="flex-1 overflow-y-auto px-8 py-6 space-y-6">
        ...
        <div class="bg-surface rounded-2xl border border-hairline p-6 shadow-xl space-y-4">
          ...
          <div class="flex gap-3 pt-2">
            ...
            <button
              ...
              class="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-elevated border border-hairline text-fg-secondary text-xs font-bold hover:bg-raised transition-all disabled:opacity-40 cursor-pointer"
            >
        ...
        <div class="bg-surface rounded-2xl border border-hairline p-6 shadow-xl space-y-4">
          ...
          <div v-if="!importPreview" class="space-y-4">
            <p class="text-xs text-fg-secondary leading-relaxed">請選擇備份的 Excel 活頁簿進行還原。系統會先載入檔案分頁與結構，顯示寫入預覽，在您確認無誤後才會執行資料庫覆寫寫入動作。</p>
            <div class="flex gap-3 flex-wrap">
              <label
                class="flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-bold transition-all cursor-pointer border border-hairline bg-elevated text-fg-secondary hover:bg-raised hover:text-fg"
          ...
          <div v-if="importPreview" class="space-y-4 border-t border-hairline pt-4">
            ...
            <p class="text-[0.6875rem] text-muted leading-relaxed">
              * 系統將使用 <code class="font-mono bg-sunken px-1 py-0.5 text-fg-secondary border border-hairline rounded text-2xs">INSERT OR REPLACE</code> 執行寫入，具有相同 ID 或主鍵的列將被<b>完全覆寫</b>。
            </p>
            <!-- 進度條（匯入中才顯示） -->
            ...
            <div class="flex items-center gap-3 pt-2">
              ...
              <button
                @click="cancelImport"
                :disabled="importingFull"
                class="px-4 py-2 rounded-xl bg-elevated border border-hairline text-fg-secondary text-xs font-bold hover:bg-raised transition-all cursor-pointer"
              >取消</button>
            </div>
          </div>
        </div>
        ...
        <div class="bg-danger/[0.04] rounded-2xl border border-danger/20 p-6 shadow-xl space-y-4">
          ...
          <p class="text-xs text-fg-secondary leading-relaxed">請勾選欲清空的資料模組。注意：清空後資料將徹底從本地庫移除且<b>無法復原</b>，執行前請確保已有 XLSX 備份。</p>
      ...
      <div class="w-full max-w-sm bg-surface rounded-2xl border border-danger shadow-2xl p-6">
        ...
        <div class="flex gap-3">
          <button
            ...
            class="flex-1 px-4 py-2 rounded-lg bg-raised hover:bg-raised text-fg-secondary text-sm"
          >取消</button>
          <button
            @click="clearLocalDb"
