...
<template>
  <div class="flex-1 overflow-y-auto bg-sunken/20 px-8 py-6 space-y-6 text-fg select-none">
    ...
    <!-- ── 外觀設定 ───────────────────────────────────────────────── -->
    <section class="bg-surface/40 backdrop-blur-md rounded-2xl border border-hairline p-5 shadow-xl space-y-4">
    ...
    <div v-if="!adminUnlocked"
      class="flex items-center gap-2.5 px-4 py-3 rounded-xl border border-hairline bg-surface/40 backdrop-blur-md text-fg-secondary text-xs select-none cursor-default w-fit shadow-md">
      <span>🔒</span>
    ...
    <section v-if="adminUnlocked" class="space-y-4">
        ...
        <div class="grid grid-cols-2 gap-4">
          <div>
            ...
            <input v-model="cloud.spreadsheetId" placeholder="請輸入試算表 ID..."
              class="w-full text-xs px-3 py-2 bg-sunken border border-hairline rounded-xl text-fg font-mono outline-none focus:border-indigo-500/50" />
          </div>
          <div>
      ...
      <div v-if="showGasHelp" class="bg-sunken border border-hairline rounded-2xl p-5 space-y-3 shadow-inner">
        ...
        <div class="relative bg-surface/60 rounded-xl border border-hairline p-4">
      ...
      <!-- Setup Guide -->
      <div v-if="showGuide" class="bg-surface/40 border border-hairline rounded-2xl p-5 space-y-4">
        <div class="flex gap-2.5 mb-2 flex-wrap border-b border-hairline pb-3">
          <button v-for="n in [1,2,3]" :key="n" @click="guideStep = n as 1|2|3"
            ...
            :class="guideStep === n ? 'bg-indigo-600 border border-indigo-500/30 text-white' : 'bg-elevated text-fg-secondary hover:text-fg'">
            步驟 {{ n }}
        ...
        <div v-if="guideStep === 1" class="space-y-3 text-xs leading-relaxed text-fg-secondary">
          ...
          <div class="bg-sunken border border-hairline rounded-xl p-4 space-y-3 font-medium">
            <div>
              ...
              <p class="text-muted mt-0.5">前往 <span class="font-mono text-indigo-400 bg-surface px-1.5 py-0.5 rounded border border-hairline">https://sheets.google.com</span> 建立一個乾淨的試算表作為排班用途。</p>
            </div>
            <div>
        ...
        <div v-if="guideStep === 2" class="space-y-3 text-xs leading-relaxed text-fg-secondary">
          ...
          <div class="bg-sunken border border-hairline rounded-xl p-4 space-y-3 font-medium">
            <div>
              ...
              <p class="text-muted mt-0.5">前往 <span class="font-mono text-indigo-400 bg-surface px-1.5 py-0.5 rounded border border-hairline">https://console.cloud.google.com</span> 並選取或建立一個新的 Cloud 專案。</p>
            </div>
            <div>
      ...
      <div class="p-5 bg-sunken rounded-2xl border border-hairline space-y-4">
        ...
        <div class="flex gap-2.5">
          <input v-model="geminiApiKeyInput" type="password"
            ...
            class="flex-1 text-xs px-3.5 py-2.5 bg-sunken border border-hairline rounded-xl text-fg font-mono outline-none focus:border-indigo-500/50" />
    ...
    <!-- ── 版本與更新 ──────────────────────────────────────────────── -->
    <section class="bg-surface/40 backdrop-blur-md rounded-2xl border border-hairline p-5 shadow-xl space-y-5">
          ...
          <div v-for="entry in changelog" :key="entry.version"
            ...
            <div class="flex items-baseline gap-3 mb-2">
              <span class="text-xs font-black font-mono tracking-wider"
                :class="entry.version === APP_VERSION ? 'text-indigo-400' : 'text-fg-secondary'">
                v{{ entry.version }}
              </span>
            ...
            <pre class="text-xs text-muted whitespace-pre-wrap font-sans leading-relaxed p-4 bg-sunken/40 border border-hairline rounded-xl font-medium">{{ entry.body || '（無更新細節說明）' }}</pre>
          </div>
        </div>
      </div>
    </section>
    ...
    <section class="bg-surface/40 backdrop-blur-md rounded-2xl border border-hairline p-5 shadow-xl space-y-4">
        ...
        <div v-if="syncWindows.length" class="space-y-2">
          <div v-for="(w, i) in syncWindows" :key="i" class="flex items-center gap-2">
            <input v-model="w.from" type="time"
              class="text-xs px-2 py-1.5 bg-sunken border border-hairline rounded-lg text-fg font-mono outline-none focus:border-indigo-500/50 w-28" />
            <span class="text-muted text-xs">→</span>
            <input v-model="w.to" type="time"
              class="text-xs px-2 py-1.5 bg-sunken border border-hairline rounded-lg text-fg font-mono outline-none focus:border-indigo-500/50 w-28" />
            <button @click="removeSyncWindow(i)" class="text-muted hover:text-rose-400 text-sm leading-none cursor-pointer transition-colors">✕</button>
          </div>
        </div>
      ...
      <div class="flex items-center gap-4 border-t border-hairline pt-4">
        ...
        <select v-model="syncIntervalHours"
          class="text-xs px-3 py-1.5 bg-sunken border border-hairline rounded-lg text-fg font-mono outline-none focus:border-indigo-500/50 cursor-pointer">
