...
<template>
  <div class="accent-indigo flex-1 overflow-y-auto bg-sunken px-8 py-6 space-y-6 text-fg select-none">

    <!-- 頁首 -->
    <div class="flex items-center justify-between border-b border-hairline pb-4">
      <div class="flex items-center gap-3">
        <span class="text-xl">⚙️</span>
        <h1 class="text-sm font-black text-fg">系統整合與設定</h1>
      </div>
    </div>

    <!-- ── 外觀設定 ───────────────────────────────────────────────── -->
    <section class="bg-surface rounded-2xl border border-hairline p-5 shadow-xl space-y-4">
      <h2 class="text-xs font-black text-muted">外觀與字型控制 (Appearance)</h2>
      <div class="flex items-center gap-6">
        <p class="text-xs text-fg-secondary font-bold w-16 shrink-0">介面字體</p>
        ...
        <p class="text-xs text-muted font-medium">調整系統側邊欄、卡片標題及主要內容面板的全域文字尺寸。</p>
      </div>

      <div class="flex items-center gap-6">
        <p class="text-xs text-fg-secondary font-bold w-16 shrink-0">介面主題</p>
        <div class="flex gap-1.5">
          <button
    ...
    <section v-if="adminUnlocked" class="space-y-4">
      <div class="flex items-center justify-between">
        <h2 class="flex items-center gap-2.5 text-xs font-black text-fg-secondary">
      ...
      <div class="bg-surface rounded-2xl border border-hairline p-6 shadow-xl space-y-4">
        ...
        <div class="flex items-center gap-3 flex-wrap pt-2 border-t border-hairline">
          ...
          <label
            class="text-xs px-4 py-2 rounded-xl border transition-colors cursor-pointer select-none font-bold text-fg-secondary border-hairline bg-elevated hover:bg-raised hover:text-fg"
          ...
          <button @click="showGasHelp = !showGasHelp" class="text-xs text-fg-secondary hover:text-fg font-bold ml-auto cursor-pointer">
            {{ showGasHelp ? '▲ 收合' : '▼ 展開' }} GAS 服務原始碼
          </button>
          <button @click="showGuide = !showGuide" class="text-xs text-fg-secondary hover:text-fg font-bold cursor-pointer">
      ...
      <!-- GAS Code -->
      <div v-if="showGasHelp" class="bg-sunken border border-hairline rounded-2xl p-5 space-y-3 shadow-inner">
        <p class="text-xs text-fg-secondary leading-relaxed font-medium">請在 Google 試算表的分頁選單中選擇 <strong class="text-fg font-bold">擴充功能 → Apps Script</strong>，新增腳本並貼入下方程式碼，接著發布部署為 <strong class="text-fg font-bold">Web 應用程式</strong>（執行身分：我、存取權限：任何人）：</p>
        <div class="relative bg-surface rounded-xl border border-hairline p-4">
      ...
      <!-- Setup Guide -->
      <div v-if="showGuide" class="bg-surface border border-hairline rounded-2xl p-5 space-y-4">
        ...
        <!-- Step 1 -->
        <div v-if="guideStep === 1" class="space-y-3 text-xs leading-relaxed text-fg-secondary">
          <h3 class="font-bold text-fg text-sm">步驟 1：建立試算表並擷取 Spreadsheet ID</h3>
          <p class="font-medium">Spreadsheet ID 是系統與雲端進行<span class="text-accent">唯讀資料拉取</span>所必需的。您僅需要將 Google Sheets 設定為任何持有連結者可讀即可。</p>
          <div class="bg-sunken border border-hairline rounded-xl p-4 space-y-3 font-medium">
            <div>
              <p class="text-fg font-bold">① 建立全新 Google 試算表</p>
              <p class="text-muted mt-0.5">前往 <span class="font-mono text-accent bg-surface px-1.5 py-0.5 rounded border border-hairline">https://sheets.google.com</span> 建立一個乾淨的試算表作為排班用途。</p>
            </div>
            <div>
              <p class="text-fg font-bold">② 從網址列解析識別 ID</p>
              <p class="text-muted mt-0.5">在試算表的瀏覽器網址列中，斜線之間的英數長字串即為試算表 ID：</p>
              <div class="font-mono text-xs bg-surface border border-hairline rounded-lg px-3 py-2 text-fg-secondary mt-1 select-all overflow-x-auto">
                https://docs.google.com/spreadsheets/d/<span class="text-warning font-bold bg-warning/10 px-1 rounded border border-warning/20">1BxiMVs0XRA5nFMdKvBdBZjgmUUqptlbs74OgVE2upms</span>/edit
              </div>
            </div>
            <div>
              <p class="text-fg font-bold">③ 建立月份分頁標籤</p>
              <p class="text-muted mt-0.5">排班資料會依據月份寫入不同的 Tab。系統會使用分頁前綴與 <code class="text-fg-secondary bg-surface font-mono px-1 rounded">YYYYMM</code> 年月結合命名。<br>
              若前綴為 <span class="font-mono text-accent">Schedule_</span>，則 2026 年 6 月的班表分頁必須命名為 <span class="font-mono text-accent font-bold">Schedule_202606</span>。</p>
            </div>
          </div>
        ...
        <!-- Step 2 -->
        <div v-if="guideStep === 2" class="space-y-3 text-xs leading-relaxed text-fg-secondary">
          <h3 class="font-bold text-fg text-sm">步驟 2：在 Google Cloud 中獲取 API Key 金鑰</h3>
          <p class="font-medium">API 金鑰是桌面端軟體快速<span class="text-accent">直連讀取</span> Google 試算表而無須繁瑣 OAuth 使用者授權的核心憑證。</p>
          <div class="bg-sunken border border-hairline rounded-xl p-4 space-y-3 font-medium">
            <div>
              <p class="text-fg font-bold">① 進入 Google Cloud 控制台</p>
              <p class="text-muted mt-0.5">前往 <span class="font-mono text-accent bg-surface px-1.5 py-0.5 rounded border border-hairline">https://console.cloud.google.com</span> 並選取或建立一個新的 Cloud 專案。</p>
            </div>
            <div>
              <p class="text-fg font-bold">② 開啟並啟用 Google Sheets API 服務</p>
              <p class="text-muted mt-0.5">在主選單點選 <span class="text-fg-secondary font-bold">API 和服務 → 程式庫</span>，搜尋「<span class="font-mono text-fg font-bold">Google Sheets API</span>」並點選啟用服務。</p>
            </div>
            <div>
              <p class="text-fg font-bold">③ 建立專屬 API 金鑰</p>
              <p class="text-muted mt-0.5">在 <span class="text-fg-secondary font-bold">API 和服務 → 憑證</span> 頁面，點選頂部「建立憑證」並選擇「<span class="text-fg-secondary font-bold">API 金鑰</span>」。<br>
              複製金鑰字串（以 <span class="font-mono text-warning font-bold">AIzaSy...</span> 開頭）填入本頁面「Google API Key」欄位中。</p>
            </div>
            <div>
              <p class="text-fg font-bold">④ 共用設定調整</p>
              <p class="text-muted mt-0.5">回到您的 Google 試算表，點選右上角「共用」，將一般存取權修改為<span class="text-fg-secondary font-bold">「知道連結的任何人」</span>可以<span class="text-fg-secondary font-bold">「檢視」</span>。</p>
            </div>
          </div>
        ...
        <!-- Step 3 -->
        <div v-if="guideStep === 3" class="space-y-3 text-xs leading-relaxed text-fg-secondary">
          <h3 class="font-bold text-fg text-sm">步驟 3：部屬 Google Apps Script (GAS) 微服務</h3>
          <p class="font-medium">由於安全限制，Google Sheets API 不允許直接在本地無登入狀態下修改試算表。我們需要透過部署一個簡單的 GAS 程式作為<span class="text-success">寫入代理門戶</span>。</p>
          <div class="bg-sunken border border-hairline rounded-xl p-4 space-y-3 font-medium">
            <div>
              <p class="text-fg font-bold">① 建立 Apps Script 專案</p>
              <p class="text-muted mt-0.5">在您的 Google 試算表中，點選上方選單的 <span class="text-fg-secondary font-bold">擴充功能 → Apps Script</span>。</p>
            </div>
            <div>
              <p class="text-fg font-bold">② 覆蓋原始碼</p>
              <p class="text-muted mt-0.5">清除預設程式碼，複製上方「GAS 服務原始碼」區塊內容並貼入，然後點選磁碟圖示存檔。</p>
            </div>
            <div>
              <p class="text-fg font-bold">③ 部署為網頁應用程式</p>
              <p class="text-muted mt-0.5">
                點選右上角 <span class="text-fg-secondary font-bold">部署 → 新增部署</span>：<br>
                • 選取類型：<span class="text-fg font-bold">網頁應用程式 (Web App)</span><br>
                • 執行身分：選擇 <span class="text-fg font-bold">我 (Me)</span><br>
                • 誰有權存取：選擇 <span class="text-fg font-bold">任何人 (Anyone)</span><br>
    ...
    <!-- ── AI 設定 ────────────────────────────────────────────────── -->
    <section class="bg-surface rounded-2xl border border-hairline p-5 shadow-xl space-y-4">
      <h2 class="text-xs font-black text-muted">人工智慧輔助設定 (AI Integrations)</h2>
      <div class="p-5 bg-sunken rounded-2xl border border-hairline space-y-4">
        <div class="flex items-center gap-2.5">
          <p class="text-xs text-fg-secondary font-bold">Gemini API Access Token</p>
          <span v-if="geminiKeySet"
            class="text-2xs font-bold px-2 py-0.5 rounded-full bg-success/10 border border-success/30 text-success shadow-[0_0_10px_rgba(16,185,129,0.05)]">
            授權有效 ✓
          </span>
          <span v-else
            class="text-2xs font-bold px-2 py-0.5 rounded-full bg-elevated border border-hairline text-muted">
            尚未填寫
          </span>
        </div>
        <div class="flex gap-2.5">
          ...
          <button v-if="geminiKeySet" @click="clearGeminiKey"
            class="text-xs px-4 py-2 border border-danger/30 text-danger hover:bg-danger/10 rounded-xl font-bold cursor-pointer transition-all">
            清除金鑰
          </button>
        </div>
    ...
    <!-- ── 版本與更新 ──────────────────────────────────────────────── -->
    <section class="bg-surface rounded-2xl border border-hairline p-5 shadow-xl space-y-5">
      <h2 class="text-xs font-black text-muted">系統更新與發佈日誌 (Version control)</h2>
      ...
      <div v-if="updateAvailable"
        class="p-5 bg-success/[0.03] border border-success/20 rounded-2xl space-y-4 shadow-[0_0_20px_rgba(16,185,129,0.02)]">
        <div class="flex items-center justify-between border-b border-success/10 pb-3">
          <div>
        ...
        <pre v-if="updateNotes" class="text-xs text-fg-secondary whitespace-pre-wrap font-sans leading-relaxed p-4 bg-sunken rounded-xl border border-hairline font-medium">{{ updateNotes }}</pre>
      </div>
      ...
      <div class="space-y-4">
        ...
        <div v-if="!changelog.length && !changelogLoading"
          class="py-8 text-center text-xs text-muted italic">
          ...
          <div v-for="entry in changelog" :key="entry.version"
            ...
            <div class="flex items-baseline gap-3 mb-2">
              <span class="text-xs font-black font-mono tracking-wider"
                :class="entry.version === APP_VERSION ? 'text-accent' : 'text-fg-secondary'">
                v{{ entry.version }}
              </span>
              <span class="text-2xs text-muted font-mono font-bold">{{ entry.date }}</span>
              <span v-if="entry.version === APP_VERSION"
                class="text-2xs font-bold px-2 py-0.5 bg-accent/10 border border-accent/30 text-accent rounded-full shadow-[0_0_10px_rgba(99,102,241,0.05)]">
                目前安裝版本
              </span>
            </div>
            <pre class="text-xs text-muted whitespace-pre-wrap font-sans leading-relaxed p-4 bg-sunken border border-hairline rounded-xl font-medium">{{ entry.body || '（無更新細節說明）' }}</pre>
          </div>
        </div>
      </div>
    </section>
    ...
    <!-- ── 雲端同步排程 ────────────────────────────────────────────── -->
    <section class="bg-surface rounded-2xl border border-hairline p-5 shadow-xl space-y-4">
      <h2 class="text-xs font-black text-muted">雲端自動同步排程 (Sync Schedule)</h2>

      <!-- 固定時段 -->
      <div class="space-y-3">
        <div class="flex items-center justify-between">
          <p class="text-xs font-bold text-fg-secondary">固定時段同步</p>
          <button @click="addSyncWindow"
            class="text-xs px-3 py-1 rounded-lg bg-elevated border border-hairline text-fg-secondary hover:text-fg hover:bg-raised transition-colors cursor-pointer font-bold">
            + 新增時段
          </button>
        </div>
        ...
        <div v-if="syncWindows.length" class="space-y-2">
          <div v-for="(w, i) in syncWindows" :key="i" class="flex items-center gap-2">
            <input v-model="w.from" type="time"
              class="text-xs px-2 py-1.5 bg-sunken border border-hairline rounded-lg text-fg font-mono outline-none focus:border-accent/50 w-28" />
      ...
      <!-- 間隔同步 -->
      <div class="flex items-center gap-4 border-t border-hairline pt-4">
        <p class="text-xs font-bold text-fg-secondary shrink-0">固定間隔同步</p>
        <select v-model="syncIntervalHours"
          class="text-xs px-3 py-1.5 bg-sunken border border-hairline rounded-lg text-fg font-mono outline-none focus:border-accent/50 cursor-pointer">
      ...
      <div class="flex items-center gap-3 flex-wrap border-t border-hairline pt-4">
        ...
        <button @click="triggerCheckVersions" :disabled="isCheckingVersions || !cloud.gasUrl"
          class="text-xs px-4 py-2 rounded-xl border border-hairline text-fg-secondary hover:text-fg hover:border-hairline disabled:opacity-40 transition-colors font-bold cursor-pointer">
    ...
    <Transition name="toast">
      <div v-if="toast"
        class="fixed bottom-6 left-1/2 -translate-x-1/2 px-5 py-2.5 bg-surface border border-hairline text-fg text-xs font-bold rounded-2xl shadow-2xl z-50 pointer-events-none backdrop-blur-md">
        {{ toast }}
      </div>
    </Transition>
